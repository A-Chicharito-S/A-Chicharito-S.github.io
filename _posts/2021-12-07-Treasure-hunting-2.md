...
'**where $\textbf{y}$ is a member of a set of well-formed outputs $\mathcal{Y}$** '. And  $\textbf{y}=\\{y_1,\,y_2,\,... \\}$ where $y\_k \in \mathcal{Y}$, $\textbf{y}_{\< t}=\\{y_1,\,y_2,\,...,y_{t-1},\,y_t\\}$. And in the following discussion, a max generation length $T$ for the sentence is considered.
