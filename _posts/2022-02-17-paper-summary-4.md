...
- **Solution**:
  ...
  <div>
  $$Loss=\lambda(t)·Loss_T+(1-\lambda(t))·Loss_S,\quad where\,\,\lambda(t)=\frac{1}{1+exp(-k·(t-t_0))},\,\,Loss_S=\frac{1}{2}\gamma\sum_i(\theta_i-\theta_i^*)^2$$
  </div>
...
For **the first setting**, the key is to *generate synthetic data* / *leverage extra data*, and methods like **back-translation** ([[here]](#2)), **self-designed algorithm** ([[here]](#4)), **domain adaption** ([[here]](#5)) are used. For **the second setting**, the key is to ***ONLY** use the original data* to enlarge the training data; thus, methods like **paraphrasing** ([[here]](#6) and [[here]](#7)) and **compressing / pruning** ([[here]](#7) and [[here]](#8)). 
